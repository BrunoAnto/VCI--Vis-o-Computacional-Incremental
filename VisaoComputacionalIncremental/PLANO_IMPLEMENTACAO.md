...
**Concluído — Sprints 0 a 5, Sprint 6 (Passo 0 e Passo 4), e Sprint 7 (Passos 1 a 5):**
...
| `c80926c` | 6 (Passo 4) | Workflow de CI/CD ajustado (nome da imagem, path de deploy, URLs) |
| `4e16d96` | 7, Passos 1 e 3 | `CLAUDE.md` do serviço + registro em `documentação/APLICACOES.md` |
| `2145f85` | 7, Passo 4 | `/simplify` (4 agentes: reuse/simplification/efficiency/altitude) aplicado |
| `498b91e` | 7, Passo 5 | `/security-review` executado e findings documentados |
| `60aeadf` | 6 (Passo 0) + 7 (Passos 5 e 6) | Porta 8060 confirmada; Correção das 3 vulnerabilidades de segurança (IDOR, permissões de grupo e PII depth) |

As 3 dúvidas de modelagem bloqueantes da Sprint 3 foram confirmadas com o dev e aplicadas: `classes_desejadas` = M2M(Classe), `anotacoes` = FileField único (zip), `caixas_detectadas` = lista de `{x, y, w, h, classe, confianca, modelo_origem}`.

**Resolução das vulnerabilidades de segurança (Sprint 7, Passo 5):**
1. **IDOR na API Dinâmica:** Mitigado em `DynamicModelPermission` e `DynamicModelViewSet` — leitura liberada a autenticados (conforme decisão do dev); alteração e exclusão estritamente restritas ao proprietário do recurso (`projeto.usuario == request.user`) ou Gerente/Superuser; criação de `Projeto` amarrada a `request.user` e anexação de filhos validada contra o dono do projeto.
2. **Permissões de grupo:** Ativadas na API Dinâmica via `DynamicModelPermission` no DRF, checando `has_perm` (`view_*`, `add_*`, `change_*`, `delete_*`) conforme `DEFAULT_GROUP_PERMISSIONS`. Usuários do perfil `Visualizador` ficam bloqueados para POST/PUT/DELETE (403 Forbidden).
3. **Vazamento de PII via `?depth=`:** Mitigado em `DynamicSerializer.build_nested_field` e `create_inline_serializer` — quando o model relacionado for `User`, expõe estritamente `['id', 'username', 'first_name', 'last_name']`, suprimindo hash de senha, `is_superuser`, `email`, permissões e grupos.

**Pendente — bloqueado por infraestrutura externa (requer VPS / ambiente compartilhado PCI):**
- [x] Confirmar porta `8060` não colide com outro serviço PCI (Sprint 6, Passo 0 — confirmado pelo dev)
...
- [ ] Verificar autenticação com a chave pública distribuída no container Docker

**Pendente — Sprint 7:**
- [x] Corrigir as 3 vulnerabilidades do `/security-review` (Passo 5)
- [x] Commitar os ajustes de segurança (Passo 6)
...
- [x] **Passo 0 (bloqueante): Confirmar a porta 8060 com o dev**

Confirmado pelo dev: a porta `8060` é a oficial do serviço e não conflita com outros serviços PCI. Mantida em `docker-compose.yml`, `CLAUDE.md` e `.env.example`.
...
- [x] **Passo 5: Revisão de segurança**
...
- `set_unusable_password()` chamado no `AuthServiceBackend`
- Sem `SECRET_KEY` ou senhas hardcoded
- `CORS_ALLOWED_ORIGINS` configurado
- Proteção contra IDOR implementada na API Dinâmica (validação de ownership)
- Permissões de grupo Django ativadas (`DynamicModelPermission`)
- Vazamento de PII suprimido em serializações aninhadas (`build_nested_field`)

- [x] **Passo 6: Commitar ajustes de qualidade**
